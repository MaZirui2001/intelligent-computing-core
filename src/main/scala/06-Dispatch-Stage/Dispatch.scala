...
class Dispatch_IO(n: Int) extends Bundle{
    ...
    val insts_disp_index    = Output(Vec(5, Vec(2, UInt(3.W))))
    val insts_disp_valid    = Output(Vec(5, Vec(2, Bool())))
}

class Dispatch extends Module{
    val io = IO(new Dispatch_IO(8))

    val queue_id_hit    = Wire(Vec(2, UInt(5.W)))
    ...
    io.insts_disp_valid     := VecInit(Seq.fill(5)(VecInit(Seq.fill(2)(false.B))))
